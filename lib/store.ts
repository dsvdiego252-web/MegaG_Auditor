...
import type {AppData,Company,Entry,ImportRecord,Rule,Snapshot,TaxMode} from './types';
...
export const ruleSchema=z.object({
  ...
  credit:z.enum(['permitir','vedar','revisar']),expectCredit:z.boolean(),
...
export const companySchema=z.object({id:z.string().regex(/^0[1-6]$/),name:z.string().trim().min(2).max(80),uf:ruleSchema.shape.uf,cnpj:z.string().regex(/^$|^\d{14}$/)});
...
export async function inputs(db:Connection,period:string) {
  ...
  return {companies,rules,imports:records.map(r=>r.payload),entries:records.flatMap(r=>r.entries)};
}
export function fingerprints(data:{companies:Company[];rules:Rule[];imports:ImportRecord[]}) {
  return {inputFingerprint:hash(JSON.stringify(data.imports.map(i=>i.id))),ruleFingerprint:hash(JSON.stringify([data.companies,data.rules]))};
}
export async function loadData(period:string):Promise<AppData> {
  ...
  return db.transaction(async tx=>{
    ...
    const periods=(await tx.query<{period:string}>('SELECT DISTINCT period FROM mega_imports ORDER BY period DESC')).map(r=>r.period);
...
export async function importFiles(items:{file:File;companyId:string;period:string;taxMode:TaxMode;emptyConfirmed:boolean;replace:boolean}[],actor:string) {
  ...
  const parsed=await Promise.all(items.map(async item=>{
    ...
    if(!result.entries.length&&!item.emptyConfirmed) throw new HttpError(item.file.name+': confirme explicitamente a ausência de movimento.');
    const record:ImportRecord={id,companyId:item.companyId,period:item.period,filename:item.file.name,hash:hash(bytes),rowCount:result.entries.length,uploadedAt:new Date().toISOString(),warnings:result.warnings,taxMode:item.taxMode,emptyConfirmed:item.emptyConfirmed};
  ...
  return db.transaction(async tx=>{
    ...
    for(const p of parsed) {
      if(!companies.some(c=>c.id===p.companyId)) throw new HttpError('Empresa inválida.');
...
export async function processPeriod(period:string,actor:string) {
  ...
  return db.transaction(async tx=>{
    await lock(tx);
    const data=await inputs(tx,period);
    if(!data.imports.length) throw new HttpError('Importe os relatórios antes de processar.');
    const result=audit(data.entries,data.rules,data.companies);
    for(const c of data.companies) if(!data.imports.some(i=>i.companyId===c.id)) result.alerts.unshift({id:'missing:'+c.id,companyId:c.id,priority:'alta',kind:'cobertura',title:'Empresa sem relatório',reason:c.name+': competência incompleta. Importe o relatório ou arquivo sem movimento confirmado.'});
    const snapshot:Snapshot={id:randomUUID(),period,processedAt:new Date().toISOString(),...result,rules:data.rules,imports:data.imports,companies:data.companies,...fingerprints(data)};
...
export async function saveCompany(input:unknown,actor:string) {
  ...
  return db.transaction(async tx=>{
    ...
    const company={...previous,...parsed,uf:parsed.uf||null,cnpj:parsed.cnpj||null};
