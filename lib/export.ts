...
export async function excelExport(data:AppData,companyId='all') {
  ...
  sheet('Resumo',['Indicador','Valor'],[
    ...
    ['Saldo dos relatórios (não é apuração final)',t.complete?t.balance/100:'Pendente: mapeamento do imposto nas saídas'],
  ...
  sheet('Regras do processamento',['CFOP','Empresa','UF','Início','Fim','Categoria','Operação','Crédito','Crédito esperado','Ativa','Fundamento','Motivo'],(snapshot?.rules||[]).map(r=>[r.cfop,r.companyId||'Todas',r.uf||'Todas',r.start,r.end||'',r.category,r.operation,r.credit,r.expectCredit?'Sim':'Não',r.active?'Sim':'Não',r.reference,r.reason]));
