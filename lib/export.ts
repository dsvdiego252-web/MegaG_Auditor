...
export async function excelExport(data:AppData,companyId='all') {
  ...
  sheet('Transferências por CFOP',['Empresa','CFOP','Direção','Valor (R$)','ICMS observado (R$)','Status','Lançamentos','Natureza','Referência','Motivos'],(snapshot?.transferCfops||[]).filter(c=>companyId==='all'||c.companyId===companyId).map(c=>[company(c.companyId),c.cfop,c.direction,c.amount/100,c.tax/100,c.status,c.entryIds.join(', '),c.description,c.reference,c.reasons.join(' | ')]));
