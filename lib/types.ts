...
export type EvaluatedEntry = Entry & { taxFindings?:TaxFinding[]; ufCheck?:UfCheck; category: Category; operation?: Operation; ruleId?: string; reasons: string[]; status: 'Conferido' | 'Revisar' };
...
export type Snapshot = { taxRules?:TaxRule[];taxPackages?:TaxPackage[]; transferCfops?:TransferCfop[]; declarations?:PeriodDeclaration[]; id: string; period: string; processedAt: string; entries: EvaluatedEntry[]; alerts: Alert[]; transfers: Transfer[]; rules: Rule[]; imports: ImportRecord[]; companies: Company[]; inputFingerprint: string; ruleFingerprint: string };
export type AppData = { taxRules?:TaxRule[];taxPackages?:TaxPackage[]; declarations?:PeriodDeclaration[]; demo: boolean; companies: Company[]; rules: Rule[]; imports: ImportRecord[]; snapshot: Snapshot | null; stale: boolean; periods: string[] };
